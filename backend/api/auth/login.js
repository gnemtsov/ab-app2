...
module.exports = (event, context, callback) => {
    FORM.isValid("login", event.arguments).then(error => {
        if (error !== undefined) {
        ...
        DB.connect()
            ...
            .then(([rows]) => {
                if (!rows.length) {
                    return callback(FORM.invalidField("u_login", "User not found."));
                } else if (rows[0].u_access !== 1) {
                    return callback(FORM.invalidField("u_login", "Access for the user is blocked."));
                } else if (!bcrypt.compareSync(u_password, rows[0].u_password)) {
                    return callback(FORM.invalidField("u_password", "Wrong password."));
                } else {

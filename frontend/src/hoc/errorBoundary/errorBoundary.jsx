...
class ErrorBoundary extends Component {
	...
	reportError = () => {
		...
		if (process.env.NODE_ENV !== "development") {
			//TODO report errors in production environment
			return;
		}
	};
	...
	componentDidCatch(error, info) {
		let errorType = "a client-side error";
		if (error.graphQLErrors) {
			errorType = "a GraphQL error";
		}
		this.setState(
			{
				errorType,
		...
		if (this.state.errorType !== undefined) {
			message =
				process.env.NODE_ENV === "development"
					? `Oops, we've got ${this.state.errorType}!\n See developer console for details...`
